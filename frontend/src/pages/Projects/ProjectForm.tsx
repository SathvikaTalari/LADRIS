...
export default function ProjectForm() {
  ...
  return <form className="card" onSubmit={submit} style={{ maxWidth: 1100, margin: '0 auto' }}>
    <h1>{edit ? 'Edit Project Source Data' : 'Create Project'}</h1>
    <p style={{ color: 'var(--color-text-muted)' }}>Enter raw project facts only. Dates, percentages, elapsed days, history rates, and ML outputs are derived by the backend/model.</p>
    {error && <div style={{ color: 'var(--color-risk-critical)', marginBottom: 12 }}>{error}</div>}
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 14 }}>
      <Field label="Project code" required><input className="input" disabled={edit} value={form.project_code} onChange={(e) => set('project_code', e.target.value)} /></Field>
      <Field label="Project name" required><input className="input" value={form.name} onChange={(e) => set('name', e.target.value)} /></Field>
      <Field label="Project type"><select className="input" value={form.project_type} onChange={(e) => set('project_type', e.target.value)}>{PROJECT_TYPES.map((value) => <option key={value}>{value}</option>)}</select></Field>
      <Field label="Acquisition act"><select className="input" value={form.acquisition_act} onChange={(e) => set('acquisition_act', e.target.value)}>{ACTS.map((value) => <option key={value}>{value}</option>)}</select></Field>
      <Field label="State code" required><input className="input" maxLength={3} value={form.state_code} onChange={(e) => set('state_code', e.target.value.toUpperCase())} /></Field>
      <Field label="Districts (comma separated)"><input className="input" value={form.district_codes} onChange={(e) => set('district_codes', e.target.value)} /></Field>
      <Field label="Implementing agency"><input className="input" value={form.executing_agency} onChange={(e) => set('executing_agency', e.target.value)} /></Field>
      {['total_area_ha','area_acquired_ha','area_in_possession_ha','total_affected_families','families_compensated','families_rehabilitated','rehabilitation_progress_pct','estimated_compensation_inr','disbursed_compensation_inr','legal_case_count'].map((key) => <Field key={key} label={key.replace(/_/g, ' ')}><input className="input" type="number" min="0" max={key === 'rehabilitation_progress_pct' ? 100 : undefined} step="any" value={form[key]} onChange={(e) => set(key, e.target.value)} /></Field>)}
      {['planned_start_date','planned_end_date','notification_3a_date','notification_3d_date'].map((key) => <Field key={key} label={key.replace(/_/g, ' ')}><input className="input" type="date" value={form[key]} onChange={(e) => set(key, e.target.value)} /></Field>)}
      ...
      <Field label="Latitude (exact only)"><input className="input" type="number" min="-90" max="90" step="any" value={form.latitude} onChange={(e) => set('latitude', e.target.value)} /></Field>
      <Field label="Longitude (exact only)"><input className="input" type="number" min="-180" max="180" step="any" value={form.longitude} onChange={(e) => set('longitude', e.target.value)} /></Field>
    </div>
